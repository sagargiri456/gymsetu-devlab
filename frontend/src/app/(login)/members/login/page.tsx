...
export default function MemberLoginPage() {
  ...
  const handleContinue = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // Check if member exists and has password
      const response = await fetch(getApiUrl("api/auth/member/check"), {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (response.ok) {
        ...
        setError(errorData.message || "Member not found. Please check your email and gym selection.");
      }
    } catch (error) {
    ...
    <main className="flex min-h-screen items-center justify-center bg-[#ecf0f3] text-gray-700 font-[Lato]">
      <div className="w-[430px] h-[550px] p-8 rounded-[35px] shadow-[6px_6px_12px_rgba(0,0,0,0.2),-6px_-6px_12px_rgba(255,255,255,0.8)] bg-[#ecf0f3]">
        ...
        <div className="mt-6 space-y-3">
          ...
          <div className="text-center text-gray-400 text-sm">
            <a href="/trainers/login" className="hover:text-[#FF8A00] transition">
              Switch to Trainer Login
            </a>
          </div>
        </div>
        ...
        <div className="text-center mt-6 text-gray-400 text-sm space-y-2">
          <div>
            <a href="/members/forgot-password" className="hover:text-[#FF8A00] transition">
              Forgot password?
            </a>
          </div>
          <div>
